...
import { genericModifier, addModifierTypes, targets as modTargets } from './modifier';
...
export const addItemTypes = (model, includeChoice = true) => {
  for (let itemType in itemTypes) {
    if (!includeChoice && itemType === 'choice') {
      continue;
    }
...
export const Item = mongoose.model('ItemGraph', officialItem);
export const ItemTC = composeMongoose(Item, composeOpts);
