...
export default {
  ...
  create: async (req: any, res: any) => {
    ...
    nClass
      ...
      .catch((err) => {
        ...
        res.status(500).send({
          message:
            err.message || 'Some error occurred when creating the class, plase try again later.',
        });
      });
  },
  ...
  getById: async (req: any, res: any, next: any) => {
    ...
    pClass
      .findOne(classReadQuery(req), '-__v')
      .then((pClass) => {
  ...
  update: async (req: any, res: any) => {
    ...
    await pClass
      .findOne(classWriteQuery(req, pClassID))
      .then(async (pClass) => {
        if (!pClass) {
