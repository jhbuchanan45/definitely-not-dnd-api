...
exports.default = exports.itemWriteQuery = exports.itemReadQuery = void 0;

var _getPermissions = require("../middlewares/getPermissions");

var _rolePermissionsControl = require("../middlewares/rolePermissionsControl");

var _item = _interopRequireDefault(require("../models/item"));

const itemReadQuery = (req, itemID = null) => (0, _rolePermissionsControl.defaultSafeReadQuery)(req, itemID, false);

exports.itemReadQuery = itemReadQuery;

const itemWriteQuery = (req, itemID = null) => (0, _rolePermissionsControl.defaultSafeWriteQuery)(req, itemID, false);

exports.itemWriteQuery = itemWriteQuery;
var _default = {
  get: async (req, res) => {
    let itemQuery = (0, _rolePermissionsControl.defaultSafeReadQuery)(req, null, false);
    ...
    _item.default.find(itemQuery, '-__v').then(items => {
  ...
  create: async (req, res) => {
    let rItem;
    const writeContentPerms = (0, _getPermissions.getWritePermissions)(req.user.permissions);
    ...
    rItem.ownerId = req.user.sub; // create new item model using schema

    const nItem = new _item.default({ ...rItem
    }); // attempt to save item

    nItem.save().then(itemDoc => {
      const {
        __v,
        ...item
      } = itemDoc.toObject(); // return newly created item to client
  ...
  getById: async (req, res, next) => {
    const itemID = req.params.itemID; // attempt to find a item with the specified ID which belongs to the requesting user

    _item.default.findOne((0, _rolePermissionsControl.defaultSafeReadQuery)(req, itemID), '-__v').then(item => {
      // if no item was found throw an error
      if (!item) {
        const error = new Error("No item with ID");
        error.kind = 'ObjectId';
        throw error;
      } // send retrieved item
  ...
  update: async (req, res, next) => {
    ...
    await _item.default.findOne((0, _rolePermissionsControl.defaultSafeWriteQuery)(req, itemID)).then(async item => {
      if (!item) {
  ...
  delete: async (req, res, next) => {
    ...
    _item.default.findOne((0, _rolePermissionsControl.defaultSafeWriteQuery)(req, itemID)).then(async item => {
